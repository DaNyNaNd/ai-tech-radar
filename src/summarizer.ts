import type { DigestJsonProvider } from './llm/types.js';
import type { RadarDigest, RadarItem } from './types.js';

export class DigestSummarizer {
  constructor(private readonly provider: DigestJsonProvider) {}
  ...
  async summarize(items: RadarItem[]): Promise<RadarDigest> {
    ...
    const jsonText = await this.provider.generateJson({
      systemPrompt,
      userPrompt,
